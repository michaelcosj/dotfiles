...
/** Commands are deliberately separate from the manager's query port. */
...
export type ChildSessionFactory = (task: SpawnTask) => Promise<ChildSession>;
...
export function createSubagentManager(
  ...
  const fold = (e: Entry, event: SubagentEvent) => {
    ...
    switch (event.type) {
      ...
      case "compaction":
        s.compactionCount++;
        s.activity = event.willRetry ? "Retrying after compaction" : "Working";
        break;
      case "delta": {
      ...
      case "meta":
        ...
        s.usage = {
          tokens: event.clearTokens
            ? event.tokens
            : (event.tokens ?? s.usage.tokens),
