...
import { SpawnError } from "./state.js";
...
export function isSuccessfulCompaction(
  ...
  // willRetry means compaction succeeded and the native session will retry the
  // overflowed request. It is not a failed compaction.
  return Boolean(event.result && !event.aborted);
}
...
export interface ChildSessionDependencies {
  /** Host model lookup is an adapter input, not part of the state model. */
...
export async function createChildSession(
  task: SpawnTask,
  dependencies?: ChildSessionDependencies,
): Promise<ChildSession> {
  const reg = dependencies?.modelRegistry;
  ...
  const settle = (forceCancelled = cancellationRequested) => {
    if (settled) return;
    settled = true;
    const last = [...session.messages]
      .reverse()
      .find((m) => role(m) === "assistant") as AssistantMessage | undefined;
    const aborted = forceCancelled || last?.stopReason === "aborted";
    const err =
      forceCancelled
        ? undefined
        : runError ||
  ...
  const unsub = session.subscribe((e: AgentSessionEvent) => {
    ...
    switch (e.type) {
      ...
      case "compaction_start":
        ...
        break;
      case "compaction_end": {
        if (isSuccessfulCompaction(e)) {
          const usage = session.getContextUsage();
          const estimatedTokens = e.result?.estimatedTokensAfter;
          emit({
            type: "compaction",
            reason: e.reason,
          ...
          emit({
            type: "activity",
            activity: e.willRetry ? "Retrying after compaction" : "Working",
          });
        } else {
  ...
  return {
    ...
    async interrupt() {
      ...
      // During compaction there may be no in-flight assistant message for
      // session.abort() to mark aborted. The explicit user cancellation still
      // must settle as cancelled rather than inheriting the prior stop reason.
