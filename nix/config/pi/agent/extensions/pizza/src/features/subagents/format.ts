...
/** Actual occupied tokens, optionally followed by the model capacity. */
export function formatContextTokens(usage: ContextUtilization) {
  const tokens = usableTokens(usage.tokens);
  const capacity = usableCapacity(usage.contextWindow);
  if (tokens === undefined)
    return capacity ? `?/${formatCompactTokens(capacity)}` : "";
  return capacity
    ? `${formatCompactTokens(tokens)}/${formatCompactTokens(capacity)} tokens`
