...
import { MAX_RUNNING, type KillResult } from "./manager.js";
...
export const BG_START_TOOL_DESCRIPTION =
  ...
  `Terminals are session-scoped: they are killed when the session ends or reloads. Output shown to you and in /ps is tail-truncated (stdout ${formatSize(STATUS_STDOUT_MAX)}, stderr ${formatSize(STATUS_STDERR_MAX)}); when persistence is available, full logs are captured to the file paths reported with truncated output. ` +
...
export const BG_START_PROMPT_GUIDELINES = [
  ...
  "After bg_start, keep working; the exit result arrives automatically. Use bg_status to peek, or bg_wait when completion is required before continuing; do not poll or run sleep.",
];
...
export function buildStartResult(snap: TerminalSnapshot) {
  return (
    ...
    `or use bg_status(id: "${snap.id}") to peek, bg_wait to await completion, bg_kill to stop it, or bg_list to see all.`
  );
}
...
function outputSection(
  ...
  if (truncation.truncated || view.truncatedBytes > 0) {
    const where = view.spillPath
      ...
      : "Full log unavailable; only the retained tail is available";
