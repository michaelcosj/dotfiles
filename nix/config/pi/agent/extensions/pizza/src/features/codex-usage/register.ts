...
export function parseCodexUsage(value: unknown): CodexUsage | undefined {
  ...
  const rawBalance = credits?.has_credits === true ? credits.balance : undefined;
  const parsedBalance = typeof rawBalance === "string"
    ? Number(rawBalance)
    : finiteNumber(rawBalance);
  const creditBalance = parsedBalance !== undefined && Number.isFinite(parsedBalance)
    ? parsedBalance
    : undefined;
  ...
  return usage.primary || usage.secondary || usage.creditBalance !== undefined
    ? usage
    : undefined;
}
...
export function formatResetTimestamp(unixSeconds: number): string {
  const reset = new Date(unixSeconds * 1_000);
  const now = new Date();
  ...
  const withinWeek = reset.getTime() > now.getTime() &&
    reset.getTime() - now.getTime() <= 7 * 86_400_000;
  if (withinWeek) {
...
export function formatCodexUsage(usage: CodexUsage): string | undefined {
  const parts = [usage.primary, usage.secondary]
    ...
    .map((window) => {
      const remaining = `${Math.round(100 - window.usedPercent)}%/${formatWindowDuration(window.windowSeconds)}`;
      return window.resetAt === undefined
        ? remaining
        : `${remaining} ↻${formatResetTimestamp(window.resetAt)}`;
    });
...
export async function fetchCodexUsage(
  ...
  if (!usage) throw new Error("Codex usage response has no rate-limit windows or credit balance");
  return usage;
}
