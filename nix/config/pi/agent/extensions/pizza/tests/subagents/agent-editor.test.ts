...
import type { SubagentReadModel } from "../../src/features/subagents/manager.ts";
import { installSubagentEditorNavigation } from "../../src/features/subagents/ui/agent-editor.ts";
...
test("composes the existing editor and only enters rows from an empty prompt", () => {
  ...
  assert.match(h.widget.render(80)[3], /⏺ First · openai\/gpt-5 · high/);
  h.editor.handleInput("\x1b[B");
