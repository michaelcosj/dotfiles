...
import {
  ...
  type SpawnTask,
  type SubagentEvent,
} from "../../src/features/subagents/state.ts";
import { createSubagentManager } from "../../src/features/subagents/manager.ts";
import type { ChildSession } from "../../src/features/subagents/child-session.ts";
...
test("folds telemetry, activity, and explicit completion reason", async () => {
  ...
  f.controls[0]!({ type: "meta", meta: {}, tokens: 12_000 });
  assert.equal(m.get(s.id)?.usage.tokens, 12_000);
  f.controls[0]!({ type: "meta", meta: {}, clearTokens: true });
  assert.equal(m.get(s.id)?.usage.tokens, undefined);
  f.controls[0]!({
    type: "compaction",
    reason: "threshold",
    willRetry: false,
  });
  assert.equal(m.get(s.id)?.activity, "Working");
  f.controls[0]!({
    type: "compaction",
    reason: "overflow",
    willRetry: true,
  ...
  assert.equal(current.usage.tokens, undefined);
  assert.equal(current.usage.contextWindow, 1000);
  assert.equal(current.activity, "Retrying after compaction");
  f.controls[0]!({
    type: "settled",
    status: "done",
    reason: "wrapped-up",
