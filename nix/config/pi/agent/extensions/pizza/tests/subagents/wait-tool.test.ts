...
import type { SubagentSnapshot } from "../../src/features/subagents/state.ts";
import type { SubagentManager, SubagentReadModel } from "../../src/features/subagents/manager.ts";
import {
  ...
  waitTreeText,
  waitWithLiveUpdates,
} from "../../src/features/subagents/ui/wait-tool.ts";
...
test("partial update is a bounded Claude-style Agents tree", () => {
  ...
  assert.match(update.content[0]!.text, /running · claude ·/);
});
...
test("wait publishes initially, throttles snapshots, and always unsubscribes", async () => {
  ...
  const waiting = waitWithLiveUpdates(
    harness.manager.view,
    (ids, signal) => harness.manager.wait([...ids], signal),
    ["sub-1"],
    undefined,
