 ...
 *
 * Architecture: plain TypeScript manager around Node child_process. Process
 * ownership, output capture, settlement, and teardown are session-scoped.
 */
...
export function registerBackgroundTerminalsExtension(pi: ExtensionAPI) {
  ...
  /** Create the session-scoped manager lazily and wire extension hooks. */
  ...
  // transition: disposeAll terminates every process tree with bounded
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal) {
      const manager = await getManager();
      const { snapshot, completed } = await manager.wait(
        params.id,
        params.timeout_ms,
        signal,
      );
      if (completed) resultDelivery.consume([snapshot.id]);
