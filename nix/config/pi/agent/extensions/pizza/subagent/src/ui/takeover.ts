...
import { formatContextTokens } from "../format.ts";
  ...
  handleInput(data: string): void {
    ...
    if (data === "x") {
      ...
      if (snap && snap.status === "running")
        void this.view.requestAbort(snap.id).catch(() => {});
      return;
    }
  }
...
class TakeoverView implements Component, Focusable {
  ...
  handleInput(data: string): void {
    if (this.keybindings.matches(data, "app.clear")) {
      ...
      if (snap?.status === "running")
        void this.view.requestAbort(this.id).catch((error) => {
          this.actionError =
            error instanceof Error ? error.message : String(error);
          this.tui.requestRender();
        });
  ...
  render(width: number): string[] {
    ...
    const rows = this.tui.terminal.rows || 30;
    const viewport = Math.max(1, rows - inputLines.length - 6);
