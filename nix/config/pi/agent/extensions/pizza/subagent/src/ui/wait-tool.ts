...
import type { SubagentManager, SubagentReadModel } from "../manager.ts";
import {
  formatStateGlyph,
  formatStateLabel,
...
export function waitTreeText(
  ...
  agents.forEach((agent, index) => {
    ...
    const metrics = formatAgentMetrics(agent, theme);
    const facts = [formatStateLabel(state, theme), activity, metrics]
      .filter(Boolean)
...
/** Wait with conversation-local partial results; aborting never requests child cancellation. */
export async function waitWithLiveUpdates(
  ...
  const publish = () => {
    ...
    if (!onUpdate) return;
    try {
      onUpdate(buildWaitUpdate(manager.view, ids));
    } catch {
  ...
  const unsubscribe = manager.view.subscribe(schedule);
  try {
    publish();
    await manager.wait([...ids], signal);
  } finally {
    if (timer) clearTimeout(timer);
    if (!unsubscribed) {
      unsubscribed = true;
